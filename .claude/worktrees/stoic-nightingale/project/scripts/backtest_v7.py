"""
Strategy v7 - Multi-Timeframe (Weekly Trend + Daily Entry)
==========================================================
Key insight: Use weekly chart for trend, daily for entry:
- Weekly: Price above 20-week MA (trend must be up)
- Daily: RSI < 30 + Volume spike
...
def get_weekly_trend(df: pd.DataFrame) -> bool:
    """Check weekly trend - use last weekly close vs 20-week MA."""
    if len(df) < 20:
        return True
    ...
    return price > sma20 if not pd.isna(sma20) else True


def get_daily_entry(df: pd.DataFrame) -> dict:
    """Daily entry signal."""
...
def run_backtest_v7():
    ...
    print("=" * 70)
    print("STRATEGY v7 - MULTI-TIMEFRAME (Weekly Trend + Daily Entry)")
    print("=" * 70)
    ...
    # Pre-compute weekly trends for each day
    print("\nComputing weekly trends...")
    weekly_trends = {}
    for date in unique_dates:
        trends = {}
        for sym, df in all_data.items():
            df_up_to_date = df[df.index <= date]
            if len(df_up_to_date) >= 20:
                trends[sym] = get_weekly_trend(df_up_to_date)
        weekly_trends[date] = trends
    ...
    for i, date in enumerate(unique_dates[30:], 1):
        ...
        if len(broker.positions) < 2:
            for sym, price in prices.items():
                ...
                sym_trends = weekly_trends.get(date, {})
                if not sym_trends.get(sym, True):
                    continue  # Weekly trend is down, skip
                ...
                signal = get_daily_entry(df_subset)
    ...
    print("\n" + "=" * 70)
    print("RESULTS - v7 (Multi-Timeframe)")
    print("=" * 70)
...
- Weekly: Price above 20-week MA (trend filter)
