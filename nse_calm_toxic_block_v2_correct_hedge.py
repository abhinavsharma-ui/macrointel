...
OUT_JSON = Path("reports/nse_calm_toxic_block_v1.json")
OUT_CSV = Path("reports/nse_calm_toxic_block_v1_trades.csv")

HEDGE_BETA = 0.25
MEDIAN_THRESHOLDS = [-5.0, -3.0, -2.0, -1.0, 0.0]
BREADTH_THRESHOLDS = [0.35, 0.40, 0.45, 0.50]
...
def build_features():
    ...
    rows = []
    for p in PRICE_DIR.glob("*.NS.parquet"):
        try:
    ...
    daily = panel.groupby("date").agg(
        nifty_ret60=("nifty_ret60", "first"),
        median_rel60=("rel60", "median"),
        pct_outperform_60=("rel60", lambda s: float((s.dropna() > 0).mean()) if len(s.dropna()) else np.nan),
        active=("rel60", "count"),
    ).reset_index()

    # Use close-t features for next executable day.
    for c in ["nifty_ret60", "median_rel60", "pct_outperform_60", "active"]:
        daily[c] = daily[c].shift(1)
    return daily

def eval_gate(trades, daily, med_th, breadth_th):
    d = trades.copy()
    ...
    toxic = (
        (d["nifty_ret60"] > 0)
        & (d["median_rel60"] < med_th)
        & (d["pct_outperform_60"] < breadth_th)
    )
    d = d[~toxic].copy()

    rel_col = "rel_net_ret" if "rel_net_ret" in d.columns else "rel_ret"
    net_col = "net_ret" if "net_ret" in d.columns else None

    if net_col and rel_col == "rel_net_ret":
        net_ret = pd.to_numeric(d[net_col], errors="coerce")
        rel_ret = pd.to_numeric(d[rel_col], errors="coerce")
        nifty_ret = net_ret - rel_ret
        d["hedged_rel_ret"] = net_ret - HEDGE_BETA * nifty_ret
    else:
    ...
    for fold, g in d.groupby("fold"):
        ...
        rows.append({
            ...
            "toxic_blocked": int(toxic.sum()),
        })
    return rows, d

def main():
    print("NSE_CALM_TOXIC_BLOCK_V1 beta=0.25 block=nifty60>0 & median_rel60<th & pct_outperform<th")
    trades = pd.read_csv(TRADES_PATH)
    daily = build_features()

    all_rows = []
    best_saved = None
    for med_th in MEDIAN_THRESHOLDS:
        for breadth_th in BREADTH_THRESHOLDS:
            rows, d = eval_gate(trades, daily, med_th, breadth_th)
            all_rows.extend(rows)
            print(f"\nMEDIAN {med_th} BREADTH {breadth_th}")
            for r in rows:
                print(f"fold={r['fold']} trades={r['trades']} rel_wr={r['rel_wr']} avg_rel={r['avg_rel']} sharpe={r['sharpe']} equity={r['equity']} blocked={r['toxic_blocked']}")
            if med_th == -3.0 and breadth_th == 0.45:
                best_saved = d

    OUT_JSON.write_text(json.dumps({"strategy": "nse_calm_toxic_block_v1", "results": all_rows}, indent=2))
