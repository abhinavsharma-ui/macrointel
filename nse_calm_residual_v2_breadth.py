...
RESIDUAL_LABEL_EDGE = 0.25
BREADTH_GATE = __import__("os").environ.get("BREADTH_GATE", "E").upper()
BREADTH_THRESH = float(__import__("os").environ.get("BREADTH_THRESH", "0.45"))
...
for fold, (train_start, train_end, test_end) in enumerate(FOLDS, 1):
    ...
    # High-VIX only
    log("  NSE_CALM_RESIDUAL_V1 ACTIVE: VIX<=20, label=residual return vs rolling beta")
    for _frame in (train_all, test_all):
        _frame["_nifty_fwd_raw"] = _frame["rel_ret"] * 0.0
        if "nifty_fwd_ret" in _frame.columns:
            _frame["_nifty_fwd_raw"] = _frame["nifty_fwd_ret"]
        elif "nifty_ret_fwd" in _frame.columns:
            _frame["_nifty_fwd_raw"] = _frame["nifty_ret_fwd"]
        elif "nifty_forward_return" in _frame.columns:
            _frame["_nifty_fwd_raw"] = _frame["nifty_forward_return"]
        _frame["beta_60"] = _frame.groupby("symbol")["rel_ret"].transform(lambda s: s.rolling(60, min_periods=20).std()).fillna(1.0)
        _frame["beta_60"] = _frame["beta_60"].clip(0.25, 2.5)
        _frame["residual_rel_ret"] = _frame["rel_ret"] - ((_frame["beta_60"] - 1.0) * _frame["_nifty_fwd_raw"])
    log(f"  NSE_CALM_RESIDUAL_V2_BREADTH ACTIVE: gate={BREADTH_GATE} breadth={BREADTH_THRESH}")
    _panel = pd.concat([train_all, test_all], ignore_index=True).copy()
    _panel["_gate_date"] = pd.to_datetime(_panel["date"]).dt.tz_localize(None).dt.normalize()
    _panel["_px"] = pd.to_numeric(_panel["adj_close"] if "adj_close" in _panel.columns else _panel["close"], errors="coerce")
    _panel = _panel.sort_values(["symbol", "_gate_date"])
    _panel["_ret20"] = _panel.groupby("symbol")["_px"].pct_change(20)
    _panel["_ret60"] = _panel.groupby("symbol")["_px"].pct_change(60)
    _bench = pd.read_parquet("data/prices_full/NIFTY50.parquet").reset_index()
    if "date" not in _bench.columns:
        _bench = _bench.rename(columns={_bench.columns[0]: "date"})
    _bench["_gate_date"] = pd.to_datetime(_bench["date"]).dt.tz_localize(None).dt.normalize()
    _bench["_bpx"] = pd.to_numeric(_bench["adj_close"] if "adj_close" in _bench.columns else _bench["close"], errors="coerce")
    _bench = _bench.sort_values("_gate_date")
    _bench["_bret20"] = _bench["_bpx"].pct_change(20)
    _bench["_bret60"] = _bench["_bpx"].pct_change(60)
    _panel = _panel.merge(_bench[["_gate_date", "_bret20", "_bret60"]], on="_gate_date", how="left")
    _panel["_rel20"] = _panel["_ret20"] - _panel["_bret20"]
    _panel["_rel60"] = _panel["_ret60"] - _panel["_bret60"]
    _daily = _panel.groupby("_gate_date").agg(
        rel_breadth_20d=("_rel20", lambda s: float((s.dropna() > 0).mean()) if len(s.dropna()) else np.nan),
        rel_breadth_60d=("_rel60", lambda s: float((s.dropna() > 0).mean()) if len(s.dropna()) else np.nan),
        xsec_rel_20d_std=("_rel20", "std"),
        xsec_rel_60d_std=("_rel60", "std"),
        active_names=("_rel20", "count"),
    ).reset_index()
    _daily["xsec_rel_20d_std_med252"] = _daily["xsec_rel_20d_std"].rolling(252, min_periods=126).median().shift(1)
    _daily["xsec_rel_60d_std_med252"] = _daily["xsec_rel_60d_std"].rolling(252, min_periods=126).median().shift(1)
    _daily["Gate_A"] = _daily["rel_breadth_20d"] >= BREADTH_THRESH
    _daily["Gate_B"] = _daily["rel_breadth_60d"] >= BREADTH_THRESH
    _daily["Gate_C"] = _daily["xsec_rel_20d_std"] >= _daily["xsec_rel_20d_std_med252"]
    _daily["Gate_D"] = _daily["xsec_rel_60d_std"] >= _daily["xsec_rel_60d_std_med252"]
    _daily["Gate_E"] = _daily["Gate_A"] & _daily["Gate_C"]
    _daily["Gate_F"] = _daily["Gate_B"] & _daily["Gate_D"]
    if BREADTH_GATE == "NONE":
        _daily["_breadth_gate_open"] = True
    else:
        _daily["_breadth_gate_open"] = _daily.get(f"Gate_{BREADTH_GATE}", False)
    _daily["_breadth_gate_open"] = _daily["_breadth_gate_open"].shift(1).fillna(False)
    train_all = train_all.merge(_daily[["_gate_date", "_breadth_gate_open"]], left_on=pd.to_datetime(train_all["date"]).dt.tz_localize(None).dt.normalize(), right_on="_gate_date", how="left").drop(columns=["key_0", "_gate_date"], errors="ignore")
    test_all = test_all.merge(_daily[["_gate_date", "_breadth_gate_open"]], left_on=pd.to_datetime(test_all["date"]).dt.tz_localize(None).dt.normalize(), right_on="_gate_date", how="left").drop(columns=["key_0", "_gate_date"], errors="ignore")
    train_all["_breadth_gate_open"] = train_all["_breadth_gate_open"].fillna(False)
    test_all["_breadth_gate_open"] = test_all["_breadth_gate_open"].fillna(False)
    train_r = train_all[train_all["vix_level"] <= VIX_THRESHOLD].copy()
    test_r = test_all[(test_all["vix_level"] <= VIX_THRESHOLD) & (test_all["_breadth_gate_open"])].copy()
    gate_pct = float(test_all["_breadth_gate_open"].mean() * 100.0) if len(test_all) else 0.0
    log(f"  residual-breadth gate={BREADTH_GATE} thresh={BREADTH_THRESH:.2f} train={len(train_r)} test={len(test_r)} gate_open={gate_pct:.1f}%")

    if len(train_r) < 200 or len(test_r) < 50:
        log(f"  FOLD {fold}: insufficient residual-calm data, skipping")
        continue

    if len(train_r) > MAX_TRAIN:
        train_r = train_r.sample(MAX_TRAIN, random_state=42+fold)

    y = (train_r["residual_rel_ret"] > RESIDUAL_LABEL_EDGE).astype(int)
    if y.nunique() < 2:
