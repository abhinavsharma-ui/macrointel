...
CALM_RANK_MIN = 0.75
CALM_RANK_COLS = [
    "momentum_sharpe_120d_rank",
    "trend_composite_rank",
    "return_120d_rank",
    "adx_rank",
]
...
for fold, (train_start, train_end, test_end) in enumerate(FOLDS, 1):
    ...
    rank_cols = [c for c in CALM_RANK_COLS if c in test_r.columns]
    if rank_cols:
        test_r["_calm_rank_score"] = test_r[rank_cols].apply(pd.to_numeric, errors="coerce").mean(axis=1)
        test_r = test_r[test_r["_calm_rank_score"] >= CALM_RANK_MIN].copy()
    log("  NSE_CALM_RANK_FILTER_V1 ACTIVE: VIX<=20, rank_score>=0.75")
    log(f"  calm-rank train={len(train_r)} test={len(test_r)}")

    if len(train_r) < 200 or len(test_r) < 50:
        log(f"  FOLD {fold}: insufficient high-VIX data, skipping")
        continue

    if len(train_r) > MAX_TRAIN:
        train_r = train_r.sample(MAX_TRAIN, random_state=42+fold)

    y = (train_r["rel_ret"] > 0.5).astype(int)
    if y.nunique() < 2:
