...
for fold, (train_start, train_end, test_end) in enumerate(FOLDS, 1):
    ...
    test_r = test_all[(test_all["vix_level"] > VIX_THRESHOLD) & (test_all["vix_spike"] > VIX_SPIKE_THRESHOLD)].copy()
    log(f"  high-VIX train={len(train_r)} test={len(test_r)}")
